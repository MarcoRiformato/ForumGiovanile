        ...
        <table class="min-w-full divide-y divide-gray-300">
            <thead>
                <tr>
                    ...
                    <th scope="col" class="py-3.5 px-4 text-left text-sm font-semibold">Data Inizio</th>
                    <th scope="col" class="py-3.5 px-4 text-left text-sm font-semibold">Data Fine</th>
                    <th scope="col" class="py-3.5 px-4 text-left text-sm font-semibold">Stato</th>
                    <th scope="col" class="py-3.5 px-4 text-left text-sm font-semibold"></th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-200">
                <tr v-for="election in elections" :key="election.id">
                    <td class="py-4 px-4 text-sm font-medium">{{ election.name }}</td>
                    <td class="py-4 px-4 text-sm">{{ election.description }}</td>
                    <td class="py-4 px-4 text-sm">{{ formatDate(election.start_date) }}</td>
                    <td class="py-4 px-4 text-sm">{{ formatDate(election.end_date) }}</td>
                    <td class="py-4 px-4 text-sm">{{ election.status }}</td>
